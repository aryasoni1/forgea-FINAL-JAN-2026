...
- Intent: Use OCI runtime semantics to ensure correct signal forwarding, readonly mounts, capability drops, and seccomp profile integration for runners.
- Important points:
  - Document how container runtimes deliver signals and map container exit codes to runner semantics (affects PASS/FAIL/ERROR classification).
  - Include example OCI runtime manifests showing seccomp profile path, capability drops, and readonly mounts for artifact directories.
  - Note differences between runtimes (runc, containerd, Firecracker) and pin runtime versions where necessary.
