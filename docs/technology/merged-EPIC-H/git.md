...
- Intent: Use Git for SHA-based checkouts, commit signature verification, and deterministic workspace preparation for verification runs.
- Important points:
  - Pin Git client versions used in runners; document `clone`/`fetch` options (shallow vs full) accepted by verification flows.
  - Document signed-commit/tag verification flows and integration with KMS/Vault for key verification and trust chains.
  - Describe immutable file protections and canonical manifest paths used by the runner (`/.forgea/verification/`).
