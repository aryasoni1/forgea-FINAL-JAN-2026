...
## EPIC-A — ALL REQUIRED OFFICIAL DOCUMENTATION

### A1_Repository & Structure — REQUIRED OFFICIAL DOCUMENTATION

- Technology: pnpm (Workspaces)
  - Official source: https://pnpm.io/workspaces
  - Exact version requirement: 10.28.1
  - Why required: Workspace discovery semantics, deterministic installs, CI frozen-lockfile enforcement.

- Technology: TypeScript (project references)
  - Official source: https://www.typescriptlang.org/docs/handbook/project-references.html
  - Exact version requirement: 5.9.3
  - Why required: Canonical compiler options and project reference behavior for monorepo builds.

- Technology: Turborepo (pipeline & caching)
  - Official source: https://turborepo.org/docs
  - Exact version requirement: VERSION UNKNOWN — MUST BE PINNED BEFORE IMPLEMENTATION
  - Why required: Pipeline schema, caching semantics and `turbo.json` schema influence CI and build determinism.

---

### A2_Package Manager & Workspace — REQUIRED OFFICIAL DOCUMENTATION

- Technology: pnpm (Workspaces & Corepack)
  - Official source: https://pnpm.io/workspaces and Node Corepack docs
  - Exact version requirement: 10.28.1
  - Why required: Defines authoritative workspace membership, `packageManager` pin, and CI install commands (`pnpm install --frozen-lockfile`).

---

### A3_Turborepo Setup — REQUIRED OFFICIAL DOCUMENTATION

- Technology: Turborepo (config schema, caching)
  - Official source: https://turborepo.org/docs and https://turborepo.org/schema.json
  - Exact version requirement: VERSION UNKNOWN — MUST BE PINNED BEFORE IMPLEMENTATION
  - Why required: `turbo` version affects `dependsOn`, `outputs`, and remote cache compatibility.

---

### A4_TypeScript Base Configuration — REQUIRED OFFICIAL DOCUMENTATION

- Technology: TypeScript `tsconfig` reference
  - Official source: https://www.typescriptlang.org/tsconfig
  - Exact version requirement: 5.9.3
  - Why required: Canonical compiler option semantics (e.g., `moduleResolution: "Bundler"`, `target: ES2022`).

---

### A5_ESLint (Minimal & Safe) — REQUIRED OFFICIAL DOCUMENTATION

- Technology: ESLint core
  - Official source: https://eslint.org/docs/latest
  - Exact version requirement: VERSION UNKNOWN — MUST BE PINNED BEFORE IMPLEMENTATION
  - Why required: Config format (Flat vs legacy), rule semantics and CI behavior.
...
- Technology: eslint-plugin-boundaries (architectural enforcement)
  ...
  - Exact version requirement: VERSION UNKNOWN — MUST BE PINNED BEFORE IMPLEMENTATION
  - Why required: Enforces import boundaries (no-cross-app imports) deterministically.

---

### A6_Prettier (Formatting Only) — REQUIRED OFFICIAL DOCUMENTATION

- Technology: Prettier
  - Official source: https://prettier.io/docs/en/index.html
  - Exact version requirement: VERSION UNKNOWN — MUST BE PINNED BEFORE IMPLEMENTATION
  - Why required: Ensures consistent formatting across editors and CI; informs `.prettierrc` and CI `--check` usage.

---

### A7_Scripts & Commands — REQUIRED OFFICIAL DOCUMENTATION

- Technology: pnpm CLI (scripts & workspace script propagation)
  - Official source: https://pnpm.io/cli/run
  - Exact version requirement: 10.28.1
  - Why required: Root `scripts` delegate to `turbo`; authoritative CLI semantics are version-sensitive.

- Technology: Turborepo (script delegation)
  - Official source: https://turborepo.org/docs
  - Exact version requirement: VERSION UNKNOWN — MUST BE PINNED BEFORE IMPLEMENTATION

---

### A8_Environment & Safety — REQUIRED OFFICIAL DOCUMENTATION

- Technology: Next.js (App Router) environment variables
  ...
  - Why required: Build-time vs runtime env semantics and `NEXT_PUBLIC_` exposure.

- Technology: dotenv (motdotla/dotenv)
  - Official source: https://github.com/motdotla/dotenv
  - Exact version requirement: 16.4.x (recommended if used)
  - Why required: Local `.env` loading and parsing semantics for developer environments.

---

### A9_Developer Experience & Guardrails — REQUIRED OFFICIAL DOCUMENTATION

- Technology: Node.js runtime & `.nvmrc` usage
  - Official sources: https://nodejs.org/en/about/releases/ and https://github.com/nvm-sh/nvm
  - Exact version requirement: VERSION UNKNOWN — MUST BE PINNED BEFORE IMPLEMENTATION (example pinned: Node 20.11.1 in briefs)
  - Why required: Reproducible developer/CI runtime and image tagging.

- Technology: EditorConfig
  - Official source: https://editorconfig.org
  - Exact version requirement: SPEC REFERENCE (pin parser/implementations if needed)

---

### A10_Documentation & Architecture — REQUIRED OFFICIAL DOCUMENTATION
...
### A11_Verification & Quality Gates — REQUIRED OFFICIAL DOCUMENTATION

- Technology: Turborepo (caching, pipeline semantics)
  - Official source: https://turborepo.org/docs/features/caching
  - Exact version requirement: VERSION UNKNOWN — MUST BE PINNED BEFORE IMPLEMENTATION

- Technology: ESLint, Prettier (gating)
  - Official sources: https://eslint.org/docs/latest | https://prettier.io/docs/en/index.html
  - Exact version requirements: VERSION UNKNOWN — MUST BE PINNED BEFORE IMPLEMENTATION

---

Notes & next steps:

- Several items above require explicit version pins (Turborepo, ESLint, Prettier, Node). Provide preferred semvers or authorize conservative pins and I will update the registry and per-EPIC `ALL_REQUIRED_DOCS.md` files accordingly.
- Recommend appending this `ALL_REQUIRED_DOCS.md` for EPIC-A to `/docs/master_docs.md` when you approve.
