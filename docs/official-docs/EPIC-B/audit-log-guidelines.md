...
## AuditLog Event Schema

- `AuditLog` entries (Prisma `AuditLog` model) must be persisted as JSONB records whose payload contains the following required keys:
  - `actorId` (UUID) — the authenticated user or service performing the action.
  - `action` (string) — the operation verb (e.g., `feature_flag.toggle`, `user.login`).
  - `entity` (string) — the resource type (e.g., `feature_flag`, `lab_session`).
  - `entityId` (UUID | string) — the target row identifier inviting traceability.
  - `source` (enum `system|api|webhook|admin`) — the origin of the mutation.
  - `metadata` (JSONB object, max 10KB) — optional, only non-PII context data (IDs, diff summaries, reason codes) may be stored; any sensitive data must be documented and redacted before insertion.
- The audit service (`packages/audit/src/audit.service.ts`) must validate the schema before each insert and reject entries that omit the required keys, exceed the 10KB metadata cap, or reference undocumented PII.
- `metadata` is stored as PostgreSQL `jsonb` so queries can filter within the payload; validation and size constraints happen in the application layer before the immutability trigger commits.

## Retention & Archival

- Primary `AuditLog` storage retains 365 days of data (hot data) in the PostgreSQL 18.1 cluster protected by the immutability triggers described elsewhere.
- Records older than 365 days are exported as newline-delimited JSON (JSONL) batches, uploaded to an S3-compatible object store configured with Object Lock in COMPLIANCE mode to enforce WORM retention, and kept for a total of 3 years (36 months) per regulatory requirements.
- Each export batch must be accompanied by a checksum and row count; only after successful hash + count verification is the primary DB allowed to delete the corresponding rows so the archive always matches the source.
- Archived JSONL files must keep the same canonical schema listed above and are considered the immutable historical record; they may be replayed for investigations but not modified.
- Deletion from the primary database occurs as part of the export job, never via a direct `DELETE` on `AuditLog` (immutability triggers prevent it), ensuring the 365-day window is maintained without violating row-level immutability.
